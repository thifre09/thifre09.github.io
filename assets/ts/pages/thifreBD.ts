...
namespace DB {
    ...
    export class Column extends Node {
        ...
        constructor(name: string, parent: Node, type: DataTypes.DataType, isPrimaryKey: boolean = false, isForeignKey: boolean = false,
            ...
            isCurrentTimestamp: boolean = false, reference?: TReference, defaultValue?: any) {
            super(name, parent);
            this.type = type;
    ...
    export class Row extends Node {
        ...
        static fromJSON(obj: any, parent: Node): Row {
            ...
            for (const [columnName, value] of Object.entries(obj.values)) {
                const column = (parent as Table).columns[columnName];
                if (column) {
...
class SGBDFunctions {
    ...
    static alterColumn(tableName: string, oldColumnName: string, newColumn: DB.Column, schemaName?: string) {
        ...
        function transformColumnToJson(column: DB.Column): string {
        ...
        if (transformColumnToJson(oldColumn) === transformColumnToJson(newColumn)) return;
        ...
        if (compareTypes(newColumn.type, types.VARCHAR(0))) {
            for (const row of table.rows) {
                ...
                if (!newColumn.type.validate(newColumn.type.parse(value))) {
                    openNotifications("<p style='color: var(--red5)'>Existem valores inválidos na coluna \"" + newColumn.name + "\". Tamanho máximo de caracteres: " + (newColumn.type as DataTypes.VarcharType).getMaxLength() + "</p>");
                    return;
                }
            }
        }
...
namespace SQL {
    ...
    export function parseColumn(columnDef: string[]): { column: DB.Column | null, error: string | null } {
        ...
        if (column.isForeignKey) {
            ...
            if (!schemaName || !tableName) return { column: null, error: "Algo deu terrivelmente errado" };
            const refTable = getCurrentDatabase()!.schemas[schemaName].tables[tableName];
...
let connectionsSvg = document.getElementById("logical-connections")! as unknown as SVGSVGElement;
...
function drawConnectionLines() {
    ...
    for (const connection of logicalConnections) {
        ...
        const headerHeight = (fromTable.element.querySelector("details > div > div") as HTMLElement).offsetHeight;
        ...
        if (fromDetails.open) {
            ...
            const summary = fromTable.element.querySelector("summary") as HTMLElement;
        ...
        if (toDetails.open) {
            ...
            const summary = toTable.element.querySelector("summary") as HTMLElement;
        ...
        for (const c of candidates) {
            ...
            if (c.fromSide !== connection.fromSide) cost += 200;
            if (c.toSide !== connection.toSide) cost += 200;
            if (c.fromSide === "right" && c.x2 < c.x1) cost += 200;
            if (c.fromSide === "left" && c.x2 > c.x1) cost += 200;
...
logical.addEventListener("mousedown", (event) => {
    if ((event.target as HTMLElement).closest(".tabela-logical")) {
        return;
    }
